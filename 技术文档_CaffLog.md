...
> 版本：v1.0 | 日期：2026-05-26 | 基于《需求文档_CoffeeMan.md》
...
├────────────────────────────────────────────┤
│            业务逻辑：Page + Utils             │
│   calculator.js / brandData.js / stats.js   │
├────────────────────────────────────────────┤
│            数据层：微信云开发                   │
│   云数据库（NoSQL 文档型）+ 云函数 + 云存储     │
└────────────────────────────────────────────┘
```
...
│   │       ├── record.svg              # 记录 tab 图标
│   │       ├── record-active.svg       # 记录 tab 选中图标
│   │       ├── dashboard.svg           # 仪表盘 tab 图标
│   │       ├── dashboard-active.svg    # 仪表盘 tab 选中图标
│   │       ├── mine.svg                # 我的 tab 图标
│   │       ├── mine-active.svg         # 我的 tab 选中图标
│   │       ├── add.svg                 # 添加记录按钮图标
│   │       ├── coffee-bean.svg         # 咖啡豆图标
│   │       ├── brand-coffee.svg        # 品牌咖啡图标
│   │       └── warning.svg             # 警告图标
│   ├── components/
...
**自制咖啡 Tab：**
1. 用户从下拉列表选择咖啡豆品种（数据源：`coffee_beans` 集合）
2. 输入咖啡豆重量（数字输入框，单位：克，范围 1-200g）
3. 从下拉列表选择冲煮方式（数据源：`brew_methods` 集合）
4. 实时计算并显示预估咖啡因含量（调用 `calculator.js`）
5. 点击"确认记录"保存到 `intake_records` 集合
...
record/index.js (Page)
  │
  ├── onLoad: 加载品种/冲煮/品牌数据到本地缓存
  │     ├── 优先读云数据库
  │     └── 降级读本地 assets/data/brand_drinks.js
  │
  ├── onSubmit: 保存记录
  │     ├── 调用 calculator.js 计算（自制）或查表（品牌）
  │     ├── 调用 recordService.addRecord()
  │     ├── 写入 intake_records 集合
...
async function getDailyTrend(days) {
  // 使用云函数聚合查询：
  // db.collection('intake_records')
  //   .aggregate()
  //   .match({ _openid: currentOpenid })
  //   .group({ _id: '$date', total: $.sum('$caffeine_mg'), count: $.sum(1) })
  //   .sort({ _id: -1 })
  //   .limit(days)
  //   .end()
}
...
exports.main = async (event, context) => {
  switch (event.action) {
    case 'addRecord':       // 添加摄入记录
    case 'getTodayRecords': // 获取当日记录
    case 'getDailyTrend':   // 获取日趋势（支持 7/30 天）
    case 'getTotalCount':   // 获取累计记录数
    case 'deleteRecord':    // 删除记录
    case 'updateRecord':    // 更新记录
    case 'clearAllRecords': // 清除所有记录
  }
};
```

**关键聚合查询示例（getDailyTrend）**：

```javascript
const $ = db.command.aggregate;
return await db.collection('intake_records')
  .aggregate()
  .match({
    _openid: wxContext.OPENID,
    recorded_at: db.command.gte(startDate)  // ISO string
  })
  .group({
    _id: $.substr(['$recorded_at', 0, 10]),  // 截取日期部分 YYYY-MM-DD
    total: $.sum('$caffeine_mg'),
    count: $.sum(1)
  })
  .sort({ _id: 1 })
  .end();
...
### 7.2 dataInit（数据初始化云函数）

在首次部署时执行，检测三个预置数据集合是否为空，若为空则批量写入：

```javascript
exports.main = async (event, context) => {
  // 1. 检查 coffee_beans 是否为空 → 写入预置品种数据
  // 2. 检查 brew_methods 是否为空 → 写入预置冲煮数据
  // 3. 检查 brand_drinks 是否为空 → 写入预置品牌饮品数据
...
- 首页仅加载当日记录和近 7 日摘要数据
- 历史数据通过分页加载（每页 20 条）
- 统计计算在云函数端用聚合查询完成，不在小程序端遍历全量数据
- 品牌饮品预置数据本地缓存一份（`assets/data/brand_drinks.js`），云数据库不可用时降级使用

### 9.3 兼容性处理

- 微信客户端版本检查：`wx.getSystemInfoSync().SDKVersion`
- 云开发能力检查：`if (!wx.cloud) { /* 降级提示 */ }`
- 弱网环境：请求超时 5s 后显示本地缓存数据

### 9.4 安全性

- 所有数据库操作通过 `_openid` 进行用户隔离（云函数中通过 `cloud.getWXContext().OPENID` 获取）
- 不使用第三方数据统计 SDK
...
| 工具 | 用途 |
|------|------|
| 微信开发者工具 | 代码编辑、实时预览、云开发管理、真机调试 |
| Git + GitHub | 版本控制（私有仓库） |
| 微信云开发控制台 | 数据库可视化管理、云函数日志 |
...
- [ ] `app.js` 中 `env` 已配置为云开发环境 ID
- [ ] 云函数 `caffeineService` 和 `dataInit` 已上传部署
- [ ] 云数据库集合 `coffee_beans`、`brew_methods`、`brand_drinks`、`intake_records` 已创建
- [ ] `dataInit` 云函数已手动触发一次，预置数据已写入
- [ ] 数据库集合权限设置为"仅创建者可读写"
